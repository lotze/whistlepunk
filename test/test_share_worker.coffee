    ...
    it "should result in effective_sharer sharing once, with three incoming users, one of whom becomes a member and one of whom becomes an invite_requested_member", (done) ->
      dbloader.db().query("select num_visits, num_members, num_invite_requested_members from shares where sharing_user_id = 'effective_sharer'").execute (error, rows, columns) ->
        if error
          console.log "ERROR: " + error
          return done(error)
        rows.length.should.eql 1, "number of shares by effective sharer"
        rows[0]['num_visits'].should.eql 3, "number of visits due to effective sharer"
        rows[0]['num_members'].should.eql 1, "number of members due to effective sharer"
        rows[0]['num_invite_requested_members'].should.eql 1, "number of invite-requested members due to effective sharer"
        done()
...
#   describe "after processing facebook like events", ->
#     before (done) ->
#       foreman = new Foreman()
#       async.series [
#         (cb) => foreman.init cb
...
#         (cb) => foreman.addWorker('share_worker', new ShareWorker(foreman), cb)
#         (cb) => dbloader.db().query("INSERT INTO olap_users (id) VALUES ('effective_sharer'),('incoming_nonmember'),('incoming_member'),('sad_sharer'),('incoming_invite_requested_member');").execute cb
#         (cb) => foreman.processFile "test/log/facebook_likes.json", cb
#       ], (err, results) =>
#         foreman.callbackWhenClear(done)
# 
#     it "should result in three shares and two sharers", (done) ->
...
#         done()
# 
#   describe "after processing invitation events", ->
#     before (done) ->
#       foreman = new Foreman()
#       async.series [
#         (cb) => foreman.init cb
...
#         (cb) => foreman.addWorker('share_worker', new ShareWorker(foreman), cb)
#         (cb) => dbloader.db().query("INSERT INTO olap_users (id) VALUES ('effective_sharer'),('incoming_nonmember'),('incoming_member'),('sad_sharer'),('incoming_invite_requested_member');").execute cb
#         (cb) => foreman.processFile "test/log/invites.json", cb
#       ], (err, results) =>
#         foreman.callbackWhenClear(done)
# 
#     it "should result in three shares and two sharers", (done) ->
...
# 
#     it "should result in one incoming member for effective_sharer", (done) ->
#       dbloader.db().query("select members_from_invitations from olap_users where id = 'effective_sharer'").execute (error, rows, columns) ->
#         if error
#           console.log "ERROR: " + error
#           return done(error)
#         assert.equal rows[0]['members_from_invitations'], 1
#         done()
# 
#     it "should result in three users not sharing at all", (done) ->
#       dbloader.db().query("select count(*) as num_nonsharers from olap_users where invitations_created=0").execute (error, rows, columns) ->
#         if error
#           console.log "ERROR: " + error
#           return done(error)
#         assert.equal rows[0]['num_nonsharers'], 3
#         done()
# 
#   describe "after processing backlogged invitations (invitations for which the invited visitors have already visited)", ->
#     before (done) ->
#       foreman = new Foreman()
#       async.series [
#         (cb) => foreman.init cb
...
#         (cb) => dbloader.db().query("INSERT INTO olap_users (id) VALUES ('effective_sharer'),('incoming_nonmember'),('incoming_member'),('sad_sharer'),('incoming_invite_requested_member');").execute cb
#         (cb) => 
#           foreman.processFile "test/log/unordered_invite_responses.json", (err, results) =>
#             foreman.callbackWhenClear(cb)
#         (cb) => foreman.processFile "test/log/backlogged_invites.json", cb
#       ], (err, results) =>
#         foreman.callbackWhenClear(done)
# 
#     it "should result in three shares and two sharers", (done) ->
